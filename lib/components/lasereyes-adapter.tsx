...
import { reactive, toRefs, getCurrentScope, onScopeDispose, Reactive, computed } from "vue";
...
const client = new LaserEyesClient(createStores(), createConfig({ network: MAINNET }));
...
export function useLaserEyes() {
  ...
  return {
    ...
    getBalance: async () => ((await client.getBalance.call(client)) ?? "").toString(),
    getInscriptions: async () => (await client.getInscriptions.call(client)) ?? [],
    ...
    requestAccounts: async () => (await client.requestAccounts.call(client)) ?? [],
